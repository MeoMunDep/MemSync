...
```bash
git clone https://github.com/MeoMunDep/MemSync.git
cd MemSync
...
```bash
docker build -t MemSync-image .
docker run -d --name MemSync-container -v $(pwd)/logs:/app/logs MemSync-image
```
...
📥 [Guide from Telegram](https://t.me/KeoAirDropFreeNee/1957)
...
```bash
docker stop MemSync-container 2>/dev/null && docker rm MemSync-container 2>/dev/null
docker build -t MemSync-image .
docker run -d --name MemSync-container -v $(pwd)/logs:/app/logs MemSync-image
```
...
```bash
docker start MemSync-container
```
...
```bash
docker stop MemSync-container
docker rm MemSync-container
docker build -t MemSync-image .
docker run -d --name MemSync-container MemSync-image
```
...
- **MemSync ** [Link](https://app.memsync.ai)
